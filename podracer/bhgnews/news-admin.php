...
if (!isset($PHP_AUTH_USER)) {
  ...
  if ($logon_id = mysql_db_query("thebhg_roster", "SELECT * FROM roster WHERE id=$PHP_AUTH_USER AND passwd=PASSWORD('$PHP_AUTH_PW');", $db_roster)) {
    if ($login_id_row = mysql_fetch_array($logon_id)) {
      $login_id_num = $login_id_row["id"];
      $login_id_name = $login_id_row["name"];
      $login_id_position = $login_id_row["position"];
      $login_id_division = $login_id_row["division"];
      ...
      if (($login_approved == 1)) {
        if ($post) {
          if ($submit) {
            ...
            $sql = "INSERT INTO podracer_newsboard (timestamp, poster, topic, message) VALUES ('$time', '$poster', '$topic', '$message');";
          ...
          if ($submit) {
            $message_db = mysql_db_query($dbname, "SELECT * FROM podracer_newsboard WHERE topicid=$edit;", $db);
            if (($login_id_num == $board_admin['id']) OR
              ...
              $topic = prepVar($topic);
              $topic = strip_tags($topic);
              $sql = "UPDATE podracer_newsboard SET topic='$topic', message='$message' WHERE topicid=$edit;";
              if (mysql_db_query($dbname, $sql, $db)) {
          ...
          if ($submit) {
            $message_db = mysql_db_query($dbname, "SELECT * FROM podracer_newsboard WHERE topicid=$delete;", $db);
            if (($login_id_num == $board_admin['id']) OR
                (($login_id_position == $board_admin['position']) AND ($login_id_division == $board_admin['division'])) OR
                ($login_id_num == mysql_result($message_db, 0, "poster"))) {
              $sql = "DELETE FROM podracer_newsboard WHERE topicid=$delete;";
              if (mysql_db_query($dbname, $sql, $db)) {
          ...
          $sql = "SELECT * FROM podracer_newsboard ORDER BY timestamp DESC";
