...
elseif (isset($delete)) {
	...
	if ($admin) {
		echo '<A HREF="' . $PHP_SELF . '?delete_ship=0&amp;id='.$id.'">Delete Ship</A> | <A HREF="' . $PHP_SELF . '?delete_ship=1&amp;id='.$id.'">Delete Ship With Refund</A><BR><BR>';
	}
	echo "<FORM NAME=\"edit\" METHOD=\"post\" ACTION=\"$PHP_SELF\"><INPUT TYPE=\"hidden\" NAME=\"id\" VALUE=\"$id\">";
	...
	foreach ($bays as $hullbay) {
		...
		if ($parts) {
			foreach ($parts as $psid=>$part) {
				...
				echo '<TD><SMALL>' . $part->GetName() . '</SMALL></TD><TD><SMALL>' . number_format($part->GetSize()) . ' ' . ($part->GetSize() == 1 ? $vol_singular : $vol_plural) . '</SMALL></TD><TD><SMALL>' . number_format($part->GetPrice()) . ' ICs</SMALL></TD><TD><SMALL><A HREF="' . $PHP_SELF . '?delete=' . $psid . '&amp;id=' . $id . '&amp;part=' . $part->GetID() . '">Delete</A></SMALL></TD></TR>';
			}
		}
		else {
