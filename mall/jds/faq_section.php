...
echo "$str_name FAQ - " . htmlspecialchars(stripslashes($sec['name']));
...
if ($q_result && mysql_num_rows($q_result)) {
	while ($q = mysql_fetch_array($q_result)) {
		echo '<HR><OL TYPE="a" START="' . ++$row . '"><LI><A NAME="' . $q['id'] . '">' . htmlspecialchars(stripslashes($q['question'])) . '</A><BR><SMALL>' . nl2br(stripslashes($q['answer'])) . '</SMALL></OL>';
	}
}
else {
