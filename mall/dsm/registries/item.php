...
echo "<A HREF=\"edit.php?id=$id\">Edit this " . $str_singular . '</A><BR>';
if ($auction = $sale->GetAuction()) {
	if ($auction->GetEnd() >= time()) {
		echo "This $str_singular is up for sale. <A HREF=\"auction-withdraw.php?id=$id\">Click here</A> to withdraw it.<HR NOSHADE SIZE=2>";
	}
	else {
		echo "The auction involving this $str_singular has finished. <A HREF=\"auction-sell.php?id=$id\">Click here</A> to sell the $str_singular or withdraw it from sale.<HR NOSHADE SIZE=2>";
	}
}
else {
	echo "<A HREF=\"auction.php?id=$id\">Auction this $str_singular</A><HR NOSHADE SIZE=2>";
}
