...
function do_form($item) {
	...
	$limit = $item->GetLimit();
	$shiponly = $item->GetShipOnly();
	$so_check = ($shiponly ? ' CHECKED' : '');
	$description = $item->GetDescription();
	echo <<<EOF1
<FORM NAME="edit_item" METHOD="post" ACTION="$PHP_SELF" ENCTYPE="multipart/form-data">
<INPUT TYPE="hidden" NAME="id" VALUE="$id">
<TABLE BORDER=0>
<TR><TD>Type:</TD><TD><SELECT NAME="type">
EOF1;
	$store = new Store();
	$types = $store->GetItemTypes();
	$cur_type = $item->GetItemType();
...
<TR><TD>Length:</TD><TD><INPUT TYPE="text" NAME="length" SIZE=5 VALUE="$length"> metres</TD></TR>
<TR><TD>Hull Strength:</TD><TD><INPUT TYPE="text" NAME="hull" SIZE=5 VALUE="$size"> RUs</TD></TR>
<TR><TD>People:</TD><TD><INPUT TYPE="text" NAME="people" SIZE=5 VALUE="$people"></TD></TR>
<TR><TD>Sales Limit:</TD><TD><INPUT TYPE="text" NAME="limit" SIZE=5 VALUE="$limit"> (0 = unlimited)</TD></TR>
<TR><TD>Sell as complete $str_singular only:</TD><TD><INPUT TYPE="checkbox" NAME="shiponly" VALUE="on"$so_check></TD></TR>
...
/*	if ($item->SetName($name)) echo 'Name saved.<BR>'; else echo 'Error saving Name.<BR>';
	...
	if ($item->SetItemType($type)) echo 'Type saved.<BR>'; else echo 'Error saving Type.<BR>';
	if ($item->SetPrice($price)) echo 'Price saved.<BR>'; else echo 'Error saving Price.<BR>';
	if ($item->SetPeople($people)) echo 'People saved.<BR>'; else echo 'Error saving People.<BR>';
	if ($item->SetLength($length)) echo 'Length saved.<BR>'; else echo 'Error saving Length.<BR>';
	if ($item->SetHullStrength($hull)) echo 'Hull Strength saved.<BR>'; else echo 'Error saving Hull Strength.<BR>';
	...
	if ($item->SetShipOnly($shiponly == 'on' ? true : false)) echo 'Ship Only saved.<BR>'; else echo 'Error saving Restriction.<BR>';*/
	if ($image) {
		if ($item->SetImage(fread(fopen($image, 'r'), filesize($image)), $image_type)) echo 'Image saved.<BR>';
		else echo 'Error saving image.<BR>';
	}
	if ($item->SetName($name) && $item->SetDescription($description) && $item->SetItemType($type) && $item->SetPrice($price) && $item->SetLength($length) && $item->SetPeople($people) && $item->SetMin($min) && $item->SetMax($max) && $item->SetRestriction($restriction) && $item->SetLimit($limit) && $item->SetShipOnly($shiponly == 'on' ? true : false) && $item->SetHullStrength($hull)) echo 'Changes saved. Click <A HREF="index.php">here</A> to return to the main menu.';
...
elseif (isset($id)) {
	...
	if ($items) {
		for ($i = 0; $i < count($items); $i++) {
			...
			$label = $type->GetName() . ': ' . $items[$i]->GetName();
			$item_op[$label] = '<OPTION VALUE=' . $items[$i]->GetID() . ' NAME="' . $items[$i]->GetName() . '">' . $label . '</OPTION>';
		}
		foreach ($item_op as $iop) {
			echo $iop;
		}
	}
	echo '</SELECT><BR><BR>';
