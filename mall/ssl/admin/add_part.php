...
function do_form() {
	...
	$position_disp_max .= '</SELECT>';
		...
		echo <<<EOF1
<FORM NAME="add_part" METHOD="post" ACTION="$PHP_SELF">
<TABLE BORDER=0>
<TR><TD>Type:</TD><TD><SELECT NAME="type">
EOF1;
	$store = new Store();
	$types = $store->GetPartTypes();
	if ($types) for ($i = 0; $i < count($types); $i++) echo '<OPTION VALUE=' . $types[$i]->GetID() . ' NAME="' . $types[$i]->GetName() . '"' . ((isset($type) && $type->GetID() == $types[$i]->GetID()) ? ' SELECTED' : '') . '>' . $types[$i]->GetName() . '</OPTION>';
	echo <<<EOF2
</SELECT>
<TR><TD>Name:</TD><TD><INPUT TYPE="text" NAME="name" SIZE=20></TD></TR>
<TR><TD>Price:</TD><TD><INPUT TYPE="text" NAME="price" SIZE=10> ICs</TD></TR>
<TR><TD>Size:</TD><TD><INPUT TYPE="text" NAME="size" SIZE=5></TD></TR>
<TR><TD>Bay:</TD><TD><SELECT NAME="bay" SIZE="1"><OPTION VALUE="0">No restriction</OPTION>
EOF2;
	$bays = $store->GetBays();
	foreach ($bays as $bay) echo '<OPTION VALUE="' . $bay->GetID() . '">' . htmlspecialchars($bay->GetName()) . '</OPTION>';
	echo <<<EOF3
</SELECT></TD></TR>
...
<TR><TD>Consumables:</TD><TD><INPUT TYPE="text" NAME="consumables" SIZE=5 VALUE="0"></TD></TR>
<TR><TD>Hull:</TD><TD><INPUT TYPE="text" NAME="hull" SIZE=5 VALUE="0"></TD></TR>
<TR><TD>Shields:</TD><TD><INPUT TYPE="text" NAME="shields" SIZE=5 VALUE="0"></TD></TR>
<TR><TD>Speed:</TD><TD><INPUT TYPE="text" NAME="speed" SIZE=5 VALUE="0"></TD></TR>
<TR><TD>Acceleration:</TD><TD><INPUT TYPE="text" NAME="acceleration" SIZE=5 VALUE="0"></TD></TR>
<TR><TD>Maneuverability:</TD><TD><INPUT TYPE="text" NAME="turnrate" SIZE=5 VALUE="0"></TD></TR>
<TR><TD>Hyperdrive:</TD><TD><INPUT TYPE="text" NAME="hyperdrive" SIZE=5 VALUE="0"></TD></TR>
<TR><TD>Power:</TD><TD><INPUT TYPE="text" NAME="power" SIZE=5 VALUE="0"></TD></TR>
</TABLE>
<INPUT TYPE="submit" VALUE="Add Part"> <INPUT TYPE="reset">
</FORM>
EOF3;
}
...
if (isset($name)) {
	...
	$type = new PartType($type);
	if ($part = $type->AddPart($name, $price, $min, $max, $restriction, $description, $limit, $size, $bay, $external == 'on')) {
		$stats = $part->GetStats();
		...
		echo 'Part added successfully. You may <A HREF="index.php">return to the main menu</A>, or add another part using the form below.<HR>';
		do_form();
	}
	else echo 'Error adding part.';
}
else do_form();
