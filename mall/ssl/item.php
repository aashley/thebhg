...
echo '<TR><TH CLASS="RIGHT">Hull Strength:</TD><TD>' . number_format($item->GetHullStrength()) . ' RU</TD></TR>';
	...
	if ($item->GetMin() == $item->GetMax()) {
		switch ($item->GetRestriction()) {
			case 1:
				...
				echo 'Must be a';
				if ($initial == 'a' || $initial == 'e' || $initial == 'i' || $initial == 'o' || $initial == 'u') echo 'n';
				echo " $name";
				break;
			case 2:
