...
function output() {
	...
	if ($sec_result && mysql_num_rows($sec_result)) {
		...
		while ($sec = mysql_fetch_array($sec_result)) {
			echo '<LI><A HREF="' . internal_link('faq_section', array('id'=>$sec['id'])) . '">' . html_escape(stripslashes($sec['name'])) . '</A>';
			...
			if ($q_result && mysql_num_rows($q_result)) {
				...
				while ($q = mysql_fetch_array($q_result)) {
					echo '<LI><A HREF="' . internal_link('faq_section', array('id'=>$sec['id']), false, $q['id']) . '">' . html_escape(stripslashes($q['question'])) . '</A>';
				}
				echo '</OL></SMALL>';
			}
		}
