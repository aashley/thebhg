...
function title() {
	return 'Administration :: Directly Award Credits';
}
...
function output() {
	...
	if ($_REQUEST['submit']) {
        ...
        continue;
			...
			if ($awardee->AddCredits($_REQUEST[$credits], $_REQUEST['reason']))
				printf('Awarded %d credits to %s.<br />', $_REQUEST[$credits], htmlspecialchars($awardee->GetName()));
			else
				printf('Error awarding %d credits to %s.<br />', $_REQUEST[$credits], htmlspecialchars($awardee->GetName()));
	...
	?>
	<input type="submit" value="Award Credits" class="button" name="submit">&nbsp;<input type="reset" class="button">
	</form>
	<?php
