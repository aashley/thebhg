...
function display(){
    ...
    $pending = $arena->Search(array('table'=>'ams_match', 'search'=>array('type'=>$activity->Get(id), 'to_date` > 0 AND `date_deleted'=>0), 'limit'=>20));
    $pendings = array();
    $status = array();
    $second = array();
    ...
    foreach ($pendings as $ja=>$match){
	    if (count($builds)){
		    ...
		    $table->AddCell('<a href="'.internal_link($page, array('op'=>'extend', 'id'=>$_REQUEST['id'], 'match'=>$match->Get(id))).'">Approve/Deny</a>');
