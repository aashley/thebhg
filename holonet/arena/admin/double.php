<?
	...
	function swap_kabal(frm) {
		...
		var person_list = eval("frm.challenger");
	...
	function swap2_kabal(frm) {
		var kabal_list = eval("frm.kabale");
...
<?php 
	...
	$form->AddTextBox('Credits:', 'chalr_cred');
	...
	if (is_object($grade)){
	    ...
	    $form->AddHidden('chalr_result', -2);
    }
	
	$form->table->StartRow();
	
	$bar_scum2 = "<select name=\"kabale\" onChange=\"swap2_kabal(this.form)\"><option value=\"-1\">N/A</option>$kabals</select>";
	
    $form->table->AddHeader(($ringa_ding2 ? $ringa_ding2 : $bar_scum2));
    
	$cell = "<select name=\"challengee\">";
	$cell .= "<option value=\"-1\" selected>N/A</option>\n";
	$cell .= "</select>";
	
	$form->table->AddHeader(($ringa_ding2 ? $bar_scum2.str_repeat('&nbsp;', 3) : '').$cell);
	$form->table->EndRow();
	$form->AddTextBox('Credits:', 'chale_cred');
