...
function display(){
    ...
    $pending = $arena->Search(array('table'=>'ams_match', 'search'=>array('type'=>$activity->Get(id), 'date_deleted'=>0, 'accepted'=>0), 'limit'=>20));
    $pendings = array();
    $status = array();
    $second = array();
    ...
    foreach ($pendings as $ja=>$match){
	    if (count($builds)){
		    ...
		    $table->AddCell('<a href="'.internal_link($page, array('op'=>'approve', 'id'=>$_REQUEST['id'], 'match'=>$match->Get(id))).'">Approve/Deny</a>');
