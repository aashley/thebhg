...
function title(){
	return 'Administration :: Coder Access :: Event Specifics Categories';
}
...
function output(){
	...
	if ($_REQUEST['op']){
		...
		if (count($current)){
			...
			$table->AddHeader('Multiple Selections');
			$table->AddHeader('&nbsp;');
			$table->AddHeader('&nbsp;');
			$table->EndRow();
			
			foreach ($current as $obj){
				$table->AddRow($obj->Get('name'), $obj->Get('description', 1), ($obj->Get('multiple') ? 'Yes' : 'No'), ($obj->Get('date_deleted') ? '<a href="'.internal_link($page, array('op'=>'ud', 'id'=>$obj->Get('id'))).'">Undelete</a>' : 
	...
	if ($show){
		...
		$form->StartSelect('Multiple', 'data[values][]', $mult);
		$form->AddOption(0, 'No');
