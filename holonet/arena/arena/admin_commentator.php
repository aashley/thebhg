<?php
function title() {
    return 'Administration :: IRC Arena :: Holonet Commentator';
}
...
function output() {
    ...
    if (isset($_REQUEST['submit'])) {
		if ($irc->NewHC($_REQUEST['bhg_id'])) {
            echo 'New Holonet Commentator Declared.';
        }
        else {
            NEC(171);
        }
        ...
        $form = new Form($page);
        $form->AddSectionTitle('Declare New Holonet Commentator');
	...
	<?
	    ...
	    $cell = "<select name=\"bhg_id\">";
			...
			$form->table->EndRow();
        $form->AddSubmitButton('submit', 'Make New Holonet Commentator');
        $form->EndForm(); 
        
        hr();
        
        $form = new Form($page);
        $form->AddSectionTitle('Downsize Current Holonet Commentator');
        $form->table->StartRow();
