<?
		...
		if (!is_array($bar_cunt)){
	    	$bar_cunt = array();
	    	foreach ($kabals_result as $kabal){
		    	$bar_cunt[] = $kabal->GetID();
	    	}	    	
    	}
    	...
    	foreach ($run as $person){
    		...
    		if ($go){
	    		if (in_array($kabal->GetID(), $bar_cunt)){
...
<?php 
	...
	for ($i = 1; $i <= $bar_maid; $i++){
	    ...
	    $form->table->AddCell($bar_scum.str_repeat('&nbsp;', 3).$cell);

	    if (!$zise){
		    $zise = 50;
	    }
	    
		$form->table->AddCell("<input type='text' name='val$i' size='$zise'>");
		$form->table->EndRow();
    }
?>