<?php
function title() {
    return 'Administration :: Arena :: Steward';
}
...
function output() {
    ...
    if (isset($_REQUEST['submit'])) {
		if ($ladder->NewSteward($_REQUEST['bhg_id'])) {
        ...
        $form = new Form($page);
        $form->AddSectionTitle('Declare New Steward');
	...
	<?
	    ...
	    $cell = "<select name=\"bhg_id\">";
			...
			$form->table->EndRow();
        $form->AddSubmitButton('submit', 'Make New Steward');
        $form->EndForm(); 
        
        hr();
        
        $form = new Form($page);
        $form->AddSectionTitle('Downsize Current Steward');
        $form->table->StartRow();
