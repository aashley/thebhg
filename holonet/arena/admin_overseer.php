<?php
function title() {
    return 'Administration :: Overseer Utilities :: Overseer';
}
...
function output() {
    ...
    if (isset($_REQUEST['submit'])) {
		if ($arena->NewOV($_REQUEST['bhg_id'])) {
        ...
        $form = new Form($page);
        $form->AddSectionTitle('Declare New Overseer');
	...
	<?
	    ...
	    $cell = "<select name=\"bhg_id\">";
			...
			$form->table->EndRow();
        $form->AddSubmitButton('submit', 'Make New Overseer');
        $form->EndForm(); 
        
        hr();
        
        $form = new Form($page);
        $form->AddSectionTitle('Retire Current Overseer');
        $form->table->StartRow();
