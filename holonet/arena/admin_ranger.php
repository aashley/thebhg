<?php
function title() {
    return 'Administration :: Survival Missions :: Rangers';
}
...
function output() {
    ...
    if (isset($_REQUEST['submit'])) {
		if ($ladder->NewRanger($_REQUEST['bhg_id'])) {
            echo 'New Ranger Declared.';
        }
        else {
            NEC(173);
        }
    } elseif (isset($_REQUEST['end'])){
		$ranger = new Ranger($ladder->CurrentRanger());
        ...
        $form = new Form($page);
        $form->AddSectionTitle('Declare New Ranger');
	...
	<?
	    ...
	    $cell = "<select name=\"bhg_id\">";
			...
			$form->table->EndRow();
        $form->AddSubmitButton('submit', 'Make New Ranger');
        $form->EndForm(); 
        
        hr();
        
        $form = new Form($page);
        $form->AddSectionTitle('Downsize Current Ranger');
        $form->table->StartRow();
