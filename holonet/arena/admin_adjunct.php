<?php
function title() {
    return 'Administration :: Overseer Utilities :: Adjunct';
}
...
function output() {
    ...
    if (isset($_REQUEST['submit'])) {
		if ($arena->NewAJ($_REQUEST['bhg_id'])) {
        ...
        $form = new Form($page);
        $form->AddSectionTitle('Declare New Adjunct');
	...
	<?
	    ...
	    $cell = "<select name=\"bhg_id\">";
			...
			$form->table->EndRow();
        $form->AddSubmitButton('submit', 'Make New Adjunct');
        $form->EndForm(); 
        
        hr();
        
        $form = new Form($page);
        $form->AddSectionTitle('Retire Current Adjunct');
        $form->table->StartRow();
