...
function bug_list_header($name, $sval, $sort, $order) {
	global $PHP_SELF;
		...
		if ($key != 'sort' && $key != 'order') {
			if (is_array($value)) {
				foreach ($value as $k=>$v) {
					$params[] = urlencode("{$key}[{$k}]") . '=' . urlencode($v);
				}
			}
			else {
	...
	echo "<TD><B><A HREF=\"$PHP_SELF?" . implode('&amp;', $params) . "&amp;sort=$sval&amp;order=";
	if ($sort == $sval) {
		echo ($order == 'a' ? 'd' : 'a');
	}
	echo "\">$name";
...
function display_bug_list($result, $show_module, $default_sort = 'module', $default_order = 'a') {
	global $sort, $order;
	...
	if (mysql_num_rows($result)) {
		...
		echo "<TR>";
		if ($show_module) {
			bug_list_header('Module', 'module', $sort, $order);
		}
		bug_list_header('Status', 'status', $sort, $order);
		bug_list_header('Priority', 'priority', $sort, $order);
		bug_list_header('Subject', 'subject', $sort, $order);
		bug_list_header('Date/Time', 'time', $sort, $order);
		bug_list_header('Last Update', 'lastupdate', $sort, $order);
		while ($bug = mysql_fetch_array($result)) {
			switch ($sort) {
				case 'module':
					$key = $bug['name'];
					break;
			...
			$bug_list[$key][] = '<TR>' . ($show_module ? "<TD><A HREF=\"$PHP_SELF?page=listbugs&module=" . $bug["module"] . "\">" . stripslashes($bug["name"]) . "</A></TD>" : '') . "<TD>" . stripslashes($bug["status"]) . "</TD><TD>" . stripslashes($bug["priority"]) . "</TD><TD><A HREF=\"$PHP_SELF?page=viewbug&bugid=" . $bug["id"] . "\">" . stripslashes($bug["subject"]) . "</A></TD><TD>" . date("j/n/Y G:i:s", $bug["time"]) . "</TD><TD>" . date("j/n/Y G:i:s", $bug["lastupdate"]) . "</TD></TR>\n";
		}
		if ($order == 'd') {
			krsort($bug_list);
		}
...
function my() {
	...
	echo "<B>New bugs in your modules</B>";
...
function dosearch() {
	...
	$sql = "SELECT bugs.id, bugs.module, modules.name, bugs.status AS sid, bugs.priority AS pid, priority.description AS priority, status.description AS status, bugs.subject, bugs.time, UNIX_TIMESTAMP(bugs.lastupdate) AS lastupdate FROM bugs, modules, priority, status WHERE modules.id=bugs.module AND priority.level=bugs.priority AND status.id=bugs.status";
	if (isset($modules)) {
