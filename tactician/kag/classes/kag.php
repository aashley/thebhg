...
class KAG {
	 ...
	 * @return boolean True on success, false otherwise.
	 */
	function AddEvent($name, $start, $end, $wstart = 0, $wend = 0, $marked = false, $content = '', $type = 0) {
		if($type != '0') {
			$tclass = new KAGType($type,$this->db);
			$name = $tclass->GetName();
		}
		$sql = 'INSERT INTO kag_events (kag, name, start, end, wstart, wend, content, type) VALUES (' . $this->id . ', "' . addslashes($name) . '", ' . ((int) $start) . ', ' . ((int) $end) . ', ' . ((int) $wstart) . ', ' . ((int) $wend) . ', "' . $content . '", ' . $type . ')';
	...
	function EmailSignups($hunter) {
		...
		if ($signups) {
			...
			$message = $hunter->GetName() . ', you have signed up for the following events in KG ' . $this->id . ".\n\n";
			$message .= implode("\n", $events);
			$message .= "\n\nYou may alter your signups at any time until " . date('j F Y \a\t G:i:s T', $this->signup_end) . ".\n\nGood luck!\n\nTatician Towers";
		}
		else {
			$message = $hunter->GetName() . ', you are no longer signed up for any events in KG ' . $this->id . ".\n\nTactician Towers";
		}
		mail($hunter->GetEmail() . ', Tactician Towers <tactician@thebhg.org>', '[KG ' . $this->id . '] Change to Signed Up Events', $message, "From: Tactician Towers <tactician@thebhg.org>\r\nReply-To: <unanswered@thebhg.org>\r\nX-Mailer: PHP");
	}
}
?>
