...
page_header('Seasonal Award for the Ladder');
	...
	if ($_REQUEST['submit']){
		...
		foreach ($_REQUEST['place'] as $pos => $reward){
			...
			if ($pos == 1){
				...
				$med = next_medal($pleb, 20);

				if (!$disable_medals && !$disable_rewards && isset($_REQUEST['domedals'])){
					$sql = 'UPDATE mb_awarded_medals SET medal=70 WHERE medal=69';
					if (mysql_query($sql, $mb->roster_db))
						echo 'Current Champion\'s Cup replaced with Mark of Valour.<br />';
					else 
						echo 'Error replacing Champion\'s Cup: ' . mysql_error($mb->roster_db) . '<br />';
					
					if ($mb->AwardMedal($pleb, $judicator, $med, 'Top Placement in Tactician Ladder Season: ' . date('M \'y', $last_month_start))) {
					...
					echo $pleb->IDline(0) . ' would be awarded a ' . $med->getName() . '.<br />';
				}
			}
		...
		$default = array(1=>100000, 2=>75000, 3=>50000, 4=>25000, 5=>15000);
		...
		$form->addCheckbox('Award Champion\'s Cup?', 'domedals', 1);
